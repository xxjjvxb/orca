...
describe('ClaimedAgentPtyOwnerRegistry', () => {
  ...
  it('generation-guards aliases and releases them with their canonical owner', async () => {
    const registry = new ClaimedAgentPtyOwnerRegistry()
    const created = await registry.ensure({
      claim: claim(),
      surface,
    ...
    const alias = claim('ccccccccccccccccccccccccccccccccccccccccccc')

    registry.addAlias('pty-1', created.owner.generation, alias)
    expect(registry.find(alias)?.ptyId).toBe('pty-1')

    registry.release('pty-1', 'old-generation')
    expect(registry.find(alias)?.ptyId).toBe('pty-1')
    registry.release('pty-1', created.owner.generation)
    expect(registry.find(alias)).toBeNull()
    expect(registry.find(claim())).toBeNull()
  })
